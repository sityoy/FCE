...
function initCitybook() {
    ...
    $(".class-bar").scrollToFixed({
        minWidth: 1064,
        marginTop: 90,
        removeOffsets: true,
        limit: function () {
            var a = $(".limit-box").offset().top - $(".class-bar").outerHeight() - 70;
            return a;
        }
    });
...
// custom
function addRangeSlider(name, spanId) {
    $('input[type="range"][name="' + name + '"].distance-radius').rangeslider({
        polyfill: false,
        onInit: function () {
            this.output = $("span#" + spanId + "").html(this.$element.val());
        },
        onSlide: function (
            position, value) {
            this.output.html(value);
