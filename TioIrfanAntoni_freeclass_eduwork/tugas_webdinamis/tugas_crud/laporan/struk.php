...
$queryy = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM pendaftaran join customer using (id_customer) where no_antrian = '$no_antrian'"); //get the data that will be updated
$dt=mysqli_fetch_array($queryy);

$per=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM pendaftaran join customer using (id_customer) where no_antrian = '$no_antrian'");
$no=1;
while($a=mysqli_fetch_array($per))
